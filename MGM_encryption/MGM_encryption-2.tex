...
\begin{tikzpicture}
    \node (n1) at (0,0) [minimum width=0.5cm, minimum height=0.75cm] {$1 \Vert \text{nonce}$};
    \node (E) at (0,-1.5) [minimum width=0.5cm, minimum height=0.75cm, draw] {$\Enc_K$};
    \foreach \x in {1, 2, 3, 4}
    {
        \node (Z\x) at ($\x*(3.5,0)+(-3.5,-3)$) [minimum width=0.5cm, minimum height=0.75cm, draw] {$Z_\x$};
        \node (E\x) [below=0.75cm of Z\x, minimum width=0.5cm, minimum height=0.75cm, draw] {$\Enc_K$};
        \node (H\x) [below=0.75cm of E\x, minimum width=0.5cm, minimum height=0.75cm, draw] {$H_\x$};
        \draw[-latex] (Z\x.south) -- (E\x.north);
        \draw[-latex] (E\x.south) -- (H\x.north);
    }
    \draw[-latex] (n1.south) -- (E.north);
    \draw[-latex] (E.south) -- (Z1.north);
    \draw[-latex] (Z1.east) -- node [above] {incr} (Z2.west);
    \draw[-latex] (Z2.east) -- node [above] {incr} (Z3.west);
    \draw[-latex] (Z3.east) -- node [above] {incr} (Z4.west);
\end{tikzpicture}
\end{document}